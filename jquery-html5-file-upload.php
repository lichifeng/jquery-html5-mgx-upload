...
class RecordUploadHandler extends UploadHandler
{
    ...
    protected function get_upload_path( $file_name = null, $version = 'mgx' ) {
        $upload_array = wp_upload_dir();
        return $upload_array['basedir'] . '/aocrecords/' . $version . date('/Y/m/') . $file_name;
    }

    protected function get_download_url( $file_name, $version = 'mgx', $direct = false ) {
        $upload_array = wp_upload_dir();
        return $upload_array['baseurl'] . '/aocrecords/' . $version . date('/Y/m/') . $file_name;
    }

    protected function set_file_name($uploaded_file, $mgx_analyzer, $record_id) {
        // Set $name
        $player_code = 'A'; // A-H
        foreach ($mgx_analyzer->players as $p) {
            $player_code = $p->owner ? chr(64+$p->index) : $player_code;
        }
        $name_string = 'Ymd_' . $mgx_analyzer->gameInfo->getPlayersString() . '_' . $record_id;
        $name = date($name_string, filemtime($uploaded_file)) . $player_code . '.' . strtolower(pathinfo($name, PATHINFO_EXTENSION));
        return $name;
    }
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error, $index = null, $content_range = null)
    {
        if ($type == 'application/octet-stream') {
            ...
            //echo $type;
            //return;
            $mgx_analyzer = new \RecAnalyst\RecAnalyst();
            $mgx_analyzer->load($name, fopen($uploaded_file, 'r'));
            if (!$mgx_analyzer->analyze()) {
                return;
            }
            ...
            // Check if host game exists
            // Get/Retrieve host game ID for record file
            if (!post_type_exists('aoc-record')) {
                return;
            }
            ...
            if (false == $host_game) {
                ...
                $host_game_attr = array(
                    ...
                    'post_content' => json_encode($mgx_analyzer->getOutput()),
                    'meta_input' => array(
                        'host_md5' => $host_md5
                    )
                );
            ...
            // Update chat messages of host game entry
...
/* Hook on ajax call */
add_action('wp_ajax_load_ajax_function', 'jqhfu_load_ajax_function'); // https://developer.wordpress.org/reference/hooks/wp_ajax__requestaction/
add_action('wp_ajax_nopriv_load_ajax_function', 'jqhfu_load_ajax_function'); // https://developer.wordpress.org/reference/hooks/wp_ajax_nopriv__requestaction/
