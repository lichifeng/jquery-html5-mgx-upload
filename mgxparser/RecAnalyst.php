...
class RecAnalyst
{
     ...
     */
    public function generateResearches()
    {
        $config = $this->config;
        ...
        $font = 12; // font used for usernames
        $real_cnt = 0;
        foreach ($this->players as $player) {
        ...
        $gd_height = ($rw + $spacing) * $real_cnt + 60;
        ...
        imagedestroy($bkgim);*/
        imagefilledrectangle( $gd, 0, 0, $gd_width, $gd_height, imagecolorallocate($gd, 0xff, 0xff, 0xff));
        ...
        foreach ($this->players as $player) {
            ...
            $dst_y = $idx * ($rh + $spacing) + $padding + round(imagefontheight($font)); $dst_x = 0 + $padding;
            $idx++;
            ...
            //imagestring() dont't support Asia characters, :(
            //imagestring($gd, $font, $dst_x + 1, $dst_y + 1, $player->name, $black);
            //imagestring($gd, $font, $dst_x, $dst_y, $player->index, $color);
            //imagettftext($gd, $font, 0, $dst_x + 1, $dst_y + 1, $black, $config->researchfontFile, $player->name);
            //imagettftext($gd, $font, 0, $dst_x, $dst_y, $color, $config->researchfontFile, $player->name);
        }*/
        ...
        foreach ($this->players as $player) {
            ...
            imagefilledrectangle( $gd, 0, $dst_y, $gd_width, $dst_y, imagecolorallocate($gd, 0xdd, 0xdd, 0xdd));
        }
        ...
        $white = imagecolorallocate($gd, 0x00, 0x00, 0x00);
        ...
        foreach ($mins as $min => $cnt) {
            ...
            $y = $real_cnt * ($rh + $spacing) + $padding + 50;
    ...
    public function getOutput() {
        ...
        return array(
            ...
            'duration'     => $this->gameTimeToString($this->gameInfo->playTime),
            'difficulty'   => $this->gameSettings->getDifficultyLevelString(),
            'map_style'    => $this->gameSettings->getMapStyleString(),
            'map_name'     => $this->gameSettings->map,
            'map_size'     => $this->gameSettings->getMapSizeString(),
            'game_type'    => $this->gameSettings->getGameTypeString(),
            'victory_mode' => $this->gameSettings->victory->getVictoryString(),
            'pop_limit'    => $this->gameSettings->popLimit,
            'teamlock'     => $this->gameSettings->lockDiplomacy ? _x('YES', 'teamlock', 'mgx-uploader') : _x('NO', 'teamlock', 'mgx-uploader'),
            'reveal_map'   => $this->gameSettings->getRevealMapString(),
            'player_num'   => count($this->players),
            'profile_num'  => count($this->playersByIndex),
            'building_stat'=> $this->buildings,
            'attendees'    => $this->players,
            'players'      => $this->playersByIndex,
            'POV'          => $this->gameInfo->getPOV(),
            'team'         => $this->teams,
